...
{
    public partial class QLSize : UserControl
    {
        ...
        public void LoadData()
        {
            ...
            dgrid_Size.Columns[2].Name = "Mã";
            dgrid_Size.Columns[3].Name = "Tên";
            dgrid_Size.Rows.Clear();
            var lstSize = _isizeService.GetAll();
            if (txt_TimKiem.Text != "")
            {
                lstSize = lstSize.Where(x => x.Ten.ToLower().Contains(txt_TimKiem.Text.ToLower())).ToList();
            }
            foreach (var x in lstSize)
            {
                dgrid_Size.Rows.Add(stt++, x.Id, x.Ma, x.Ten);
            }
        }
        private void Clear()
        {
        ...
        {
                _id = Guid.Parse(dgrid_Size.CurrentRow.Cells[1].Value.ToString());
                var x = _isizeService.GetAll().FirstOrDefault(x => x.Id == _id);
                txt_Ma.Text = x.Ma;
                txt_Ten.Text = x.Ten;
        }
        ...
        private void txt_TimKiem_TextChanged(object sender, EventArgs e)
        {
            ...
            dgrid_Size.Columns[3].Name = "Tên";
            dgrid_Size.Rows.Clear();
            var lstMauSac = _isizeService.GetAll();
            if (txt_TimKiem.Text != "")
            {
        ...
        private void rbtn_Xoa_Click_1(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    var size = context.Sizes.Include(s => s.SanPhamChiTiets).SingleOrDefault(s => s.Id == _id);

                    if (size != null && size.SanPhamChiTiets.Any())
                    {
